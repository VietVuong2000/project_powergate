...
const MainAddNew = () => {
  ...
  const getDataUpdate = async () => {
    try {
      ...
      // dispatch(getdatas(res.data.data))
      // setDisplayBtn('block')
      // setDisplayBtnAdd('none')
    }
    catch (err) {
      console.log(err)
    }
    ...
    <div style={{
      ...
      <div className="headContent" style={{ width: '90%' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center' }}>
          {/* <Button className='button-addemployee' variant="outlined" disabled={true} style={{ display: `${displayBtnAdd}` }}>Add</Button> */}
          <Button className='button-addemployee' variant="outlined" >save Change</Button>
        </div>
      </div>
      <div style={{ height: '90%' }}>
        <Box sx={{ width: '100%' }}>
          ...
          <TabPanel value={value} index={0}>
            <ContentAddNew dataDetail={dataDetail} />
          </TabPanel>
          <TabPanel value={value} index={1}>
            <FormContactInformation dataDetail={dataDetail}/>
          </TabPanel>
          <TabPanel value={value} index={2}>
            <FormEmloyeeDetail dataDetail={dataDetail}/>
          </TabPanel>
          <TabPanel value={value} index={3}>
            <FormSalary dataDetail={dataDetail}/>
          </TabPanel>
          <TabPanel value={value} index={4}>
            <FormOrther dataDetail={dataDetail}/>
          </TabPanel>
        </Box>
      </div>
